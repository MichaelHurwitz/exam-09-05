let militaryUnit = {
  ...
  const Mission2 = (unit) => {
    const soldiarsCount = unit.personnel.length;
    return soldiarsCount
  }
  ...
  const Mission3 = (newDeployment, unit) => {
    ...
    unit.history.push(lastDeployment);
  ...
  const Mission4 = (newFirearm, unit) => {
    const existingFirearm = unit.equipment.firearms.find(firearm => 
